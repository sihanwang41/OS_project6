...
int ext3_set_gps_location(struct inode *inode)
{
	...
	coord_age = get_current_loc(&loc);

	spin_lock(&ei->gps_lock);
	memcpy(&ei->i_latitude, &loc.latitude, sizeof(loc.latitude));
	memcpy(&ei->i_longitude, &loc.longitude, sizeof(loc.longitude));
	memcpy(&ei->i_accuracy, &loc.accuracy, sizeof(loc.accuracy));
	ei->i_coord_age = coord_age;
	spin_unlock(&ei->gps_lock);
...
int ext3_get_gps_location(struct inode *inode, struct gps_location *loc)
{
	...
	spin_lock(&ei->gps_lock);
	memcpy(&loc->latitude, &ei->i_latitude, sizeof(ei->i_latitude));
	memcpy(&loc->longitude, &ei->i_longitude, sizeof(ei->i_longitude));
	memcpy(&loc->accuracy, &ei->i_accuracy, sizeof(ei->i_accuracy));
	coord_age = ei->i_coord_age;
	spin_unlock(&ei->gps_lock);
